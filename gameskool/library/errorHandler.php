	...
	class errorHandler {
		...
		';
			...
			if (!in_array($uniqueError, $excessiveErrors)) {
				$sql = "INSERT INTO `errors` (
							`class`, `code`, `type`, `file`, 
							`line`, `function`, `message`, 
							`date`, `trace`, `lastModified` 
						) VALUES (
							'error', '".prep($errno)."', '".prep($errorTypes[$errno])."', '".prep($errfile)."', 
							'".prep($errline)."', '".(isset($backTrace[$surface]['function']) ? prep($backTrace[$surface]['function']) : '')."', '".prep($errstr)."', 
							CURDATE(), '".prep(preg_replace('/\t\t/', '', $traceString))."', NOW()
						) ON DUPLICATE KEY UPDATE `count` = `count` + 1, 
						`status` = IF(`count` > 100, 'overflow', `status`)";
				query($sql);
			}
			...
			switch ($errno) {
				case E_USER_ERROR:
					if (isDevEnvironment()) {
						...
						redirect('/error/status/code/500');
					}
					break;
				case E_USER_WARNING:
					self::printError('Error', $errno, $errorTypes[$errno], $errfile, $errline, $errstr, $traceString);
		 ...
		 */
		 static function retrieveErrorOverflow() {
			$result = query("SELECT `class`, `code`, `file`, `line`, `function` FROM `errors` WHERE `status` IN ('overflow', 'uncaughtoverflow')");
